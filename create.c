...
char	*create_d(t_arg arg)
{
	...
	if (arg.width > (int)ft_strlen(n) || arg.accur > (int)ft_strlen(n))
	{
		if (ft_strchr(arg.setting, '-'))
		{
			...
			while (arg.accur-- > (int)ft_strlen(n))
				ft_strncat(res, "0", 1);
			ft_strcat(res, n);
			while (arg.width-- > (int)(ft_strlen(n) + (ft_strchr(arg.setting, '+') || ft_strchr(arg.setting, ' ') || n[0] == '-') + (((int)ft_strlen(n) >= accur) ? 0 : (accur - ft_strlen(n)))))
				ft_strncat(res, " ", 1);
		}
		else
		{
			accur += (ft_strchr(arg.setting, '+') || n[0] == '-' || ft_strchr(arg.setting, ' ')) && (!ft_strchr(arg.setting, '0') || arg.accur != -1) ? 1 : 0;
			if ((ft_strchr(arg.setting, '+') || n[0] == '-' || ft_strchr(arg.setting, ' ')) && (ft_strchr(arg.setting, '0') && arg.accur == -1))
				ft_strncat(res, (n[0] == '-') ? "-" : (ft_strchr(arg.setting, '+') ? "+" : " "), 1);
			while (arg.width > accur && arg.width-- > (int)ft_strlen((n[0] == '-') ? n + 1 : n) + ((ft_strchr(arg.setting, '+') || ft_strchr(arg.setting, ' ') || n[0] == '-') && accur <= (int)ft_strlen(n) + (n[0] == '-')))
				ft_strncat(res, (ft_strchr(arg.setting, '0') && arg.accur == -1 ? "0" : " "), 1);
			if ((ft_strchr(arg.setting, '+') || n[0] == '-' || ft_strchr(arg.setting, ' ')) && (!ft_strchr(arg.setting, '0') || arg.accur != -1))
				ft_strncat(res, (n[0] == '-') ? "-" : (ft_strchr(arg.setting, '+') ? "+" : " "), 1);
			while (accur-- > (int)ft_strlen((n[0] == '-') ? n + 1 : n) + (ft_strchr(arg.setting, '+') || ft_strchr(arg.setting, ' ') || n[0] == '-'))
				ft_strncat(res, "0", 1);
	...
	{
		if (ft_strchr(arg.setting, '+') || n[0] == '-' || ft_strchr(arg.setting, ' '))
			ft_strncat(res, (n[0] == '-') ? "-" : (ft_strchr(arg.setting, '+') ? "+" : " "), 1);
		ft_strcat(res, (n[0] == '-' ? n + 1 : n));
	}
	return (res);
...
char	*create_un(t_arg arg, int base)
{
	...
	if (arg.width > len || arg.accur > len)
	{
		if (ft_strchr(arg.setting, '-'))
		{
			if (ft_strchr(arg.setting, '0') && ft_strchr(arg.setting, '#') && (ft_strchr(arg.type, 'x') || ft_strchr(arg.type, 'X')))
				ft_strncat(res, "0x", 2);
			while (arg.accur-- > len + (ft_strchr(arg.setting, '#') ? 2 : 0))
				ft_strncat(res, "0", 1);
			ft_strcat(res, n);
			while (arg.width-- > ((accur > len) ? accur : len) + (ft_strchr(arg.setting, '#') ? 2 : 0))
				ft_strncat(res, " ", 1);
		}
		else
		{
			if (ft_strchr(arg.setting, '0') && accur == -1 && ft_strchr(arg.setting, '#') && (ft_strchr(arg.type, 'x') || ft_strchr(arg.type, 'X')))
				ft_strncat(res, "0x", 2);
			while (arg.width-- > ((accur > len) ? accur : len) + (ft_strchr(arg.setting, '#') ? 2 : 0))
