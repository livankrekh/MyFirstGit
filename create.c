	...
	if (arg.width > (int)ft_strlen(n) || arg.accur > (int)ft_strlen(n))
	{
		if (ft_strchr(arg.setting, '-'))
		{
			...
			while (arg.accur-- > (int)ft_strlen(n))
				ft_strncat(res, "0", 1);
			ft_strcat(res, n);
			while (arg.width-- > (int)(ft_strlen(n) + (ft_strchr(arg.setting, '+') || ft_strchr(arg.setting, ' ')) + (((int)ft_strlen(n) >= accur) ? 0 : (accur - ft_strlen(n)))))
				ft_strncat(res, " ", 1);
		}
		else
		{
	...
	if (arg.width > len || arg.accur > len)
	{
		if (ft_strchr(arg.setting, '-'))
		{
			while (arg.accur-- > len)
				ft_strncat(res, "0", 1);
			ft_strcat(res, n);
			while (arg.width-- > ((accur > len) ? accur : len))
				ft_strncat(res, " ", 1);
		}
		else
		{
			if (ft_strchr(arg.setting, '0') && accur == -1 && ft_strchr(arg.setting, '#'))
				ft_strncat(res, "0x", 2);
